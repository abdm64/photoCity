...
class PhotoVC: UIViewController {
    ...
    func configureCell(){
        cellWidth = floor(screenSize.width * cellScalling)
        cellHight = floor(screenSize.height * cellScalling)
        let insetX = (view.bounds.width - cellWidth) * 0.5
        let insetY = (view.bounds.height - cellHight ) * 0.5
...
extension PhotoVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "Cell", for: indexPath)  as? InteresetPhotoCell else { return UICollectionViewCell() }
            //let imageFromIndex = imageArray[indexPath.row]
            //cell.imageView = UIImageView(image: imageFromIndex)
           // cell.imageView.downloadedFrom(link: imageUrlArray[indexPath.row])
        
        
           // cell.imageView.sd_setImage(with: URL(string:imageUrlArray[indexPath.row]), completed: nil)
